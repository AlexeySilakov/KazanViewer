function [y, ax] = kv_hyscorefd(Sys, Exp, Opt, varargin)
...
%   Opt.Symmetry = 'Ci'; Opt.nKnots = 20; 
%   Opt.OriSelFile = 'orisel.mat'; alternatively, one can load pregenerated grid, saved in .mat file
%           it should contain three parameters : "phi", "theta" and "weights" 
%   Opt.Verbosity = 1;
...
tDead = safeget(Exp, 'tDead', 0)*1e-9; %ns->s
%%% simulation for a few nuclei
if isfield(Sys, 'TA')
    Parameters ={'Q','Qpa', 'Nucs', 'gn', 'I', 'nNucs'};
else
...
for pk = 1:length(Parameters)
    if isfield(Sys, Parameters{pk})        
        ...
        if ~strcmp(Parameters{pk}, 'A') %%% exclude because it can be a full matrix
            if size(tPar, 1)>1
                sim_size = size(tPar, 1);
            end
        end
        if ~strcmp(Parameters{pk}, 'I')
            if length(tPar)>1
                sim_size = length(tPar);
            end
        end
    end
end
...
if sim_size>1
    ...
    for kk = 1:length(isfield_num)
        ...
        elseif size(tPar, 1)~=sim_size
            if strcmp(Parameters{pk}, 'A')
                if size(tPar, 1)==sim_size*3
                    Sys = setfield(Sys, Parameters{pk}, tPar);
                else
...
%%VVVVVVV allow external grid , alsi 29.02.2012
grid_outside = 0;
if isstruct(ORI)
    if isfield(ORI, 'phi') & isfield(ORI, 'theta')
        phi = ORI.phi;
        theta = ORI.theta;
        weigths = safeget(ORI, 'weigths', phi*0+1);
        grid_outside = 1;
    else
        disp('kv_hyscorefd, WARNING: specified grid is not complete. Switching to the internal procedures');
    end
end
...
%%AAAAAAA allow external grid , alsi 29.02.2012  
...
Amps = [];
nNucs = Sys.nNucs;
make_prod = safeget(Opt, 'ProdRule', 0);
...
for ii  =1:sum(nNucs)
    ...
    %%%% add an option to specify exact HF matrix
    if isfield(Sys, 'TA')
        if iscell(Sys.TA)
            if numel(Sys.TA)~=sum(nNucs)
                error('Sys.TA does not have the correct number of cells')
            end
        ...
        if size(Sys.A, 1) == sim_size*3
            A = Sys.A([1:3]+(ii-1), :)*1e6;
        else
...
%       Quadrupole coupling 
    ...
    if isfield(Sys, 'TQ')
        if iscell(Sys.TQ)
            if numel(Sys.TQ)~=sum(nNucs)
                errot('Sys.TA does not have correct number of cells')
            end
    ...
    nstones = ones(mult, 1);
    tii = reshape(b1(:, nstones, nstones, nstones, nstones, nstones), mult^6, 1);
    tkk = reshape(b2(nstones, :, nstones, nstones, nstones, nstones), mult^6, 1);
    tll = reshape(b3(nstones, nstones, :, nstones, nstones, nstones), mult^6, 1);
    tnn = reshape(b4(nstones, nstones, nstones, :, nstones, nstones), mult^6, 1);
    tjj = reshape(b5(nstones, nstones, nstones, nstones, :, nstones), mult^6, 1);
    tmm = reshape(b6(nstones, nstones, nstones, nstones, nstones, :), mult^6, 1);
...
MatrA =  complex(zeros(Exp.nPoints*2));
...
%     for ci = 1:sim_size  
    for ci = Opt.Nuclei
        ...
        [V_a, tD_a] = eig(HamA);  D_a = diag(tD_a); 
        [V_b, tD_b] = eig(HamB);  D_b = diag(tD_b);
        Str.V_a = V_a;
        Str.V_b = V_b;
        Str.E_a = D_a;
        Str.E_b = D_b;
...
% Amp = W*0+1;
% %%%%%%%%%%%%%%%%%%%% -------------- test
        [Amp, W] = Local_crosspeak(Str, tau, cii{ci}, ckk{ci}, cll{ci}, cnn{ci}, cjj{ci}, cmm{ci}, tDead);
    ...
    binHYSCORE_new(MatrA, aFreqs(:, 1), aFreqs(:, 2), complex(aAmps(:, 1)), Exp.MaxFreq); %alsi 2020... new binHYSCORE (see comment in binHYSCORE_new.c)
    binHYSCORE_new(MatrA, aFreqs(:, 2), aFreqs(:, 1), complex(aAmps(:, 2)), Exp.MaxFreq);
    
%     MatrA = binHYSCORE(MatrA, aFreqs(:, 1), aFreqs(:, 2), aAmps(:, 1), Exp.MaxFreq); %alsi 2020... 
%     MatrA = binHYSCORE(MatrA, aFreqs(:, 2), aFreqs(:, 1), aAmps(:, 2), Exp.MaxFreq);
end    
% disp(cputime - tstart);
MatrA(1, 1) = MatrA(1, 1); 

kill_neg = safeget(Opt, 'KillNeg', 1);
dx = Exp.MaxFreq*2/Exp.nPoints;
% MatrA = make_fftcomb_bin(Freqs, Amps, Exp, kill_neg, safeget(Sys, 'lw', 0)*1e6/dx);  %alsi 08.06.06

MatrA = make_fft(MatrA, Exp, kill_neg, safeget(Sys, 'lw', 0)*1e6/dx);

ax.x = linspace(-Exp.MaxFreq, Exp.MaxFreq, Exp.nPoints).'; % Hz
ax.y = linspace(-Exp.MaxFreq, Exp.MaxFreq, Exp.nPoints).'; % Hz

if safeget(Opt, 'DoNorm', 0)
...
%----------------------------------------------------
function MatrA = make_fft(MatrA, Exp, kill_neg, lw)
% MatrA = abs(MatrA) + abs(MatrA.');
kill_zeros = 1; %%%%%%%%%%% to remove negative times
use_LoGa = 0;   %%%%%%%%%%% to make lorentz-gauss transofrmation / remove "stars"
[A, B] =size(MatrA) ;
if kill_zeros
        gauss = gaussian(1:(A), A/2, 1);
        gauss = 1-gauss/max(gauss);
        Killing_mashine = gauss(ones(A, 1), :).*gauss(ones(A, 1), :).';
        MatrA = MatrA.*Killing_mashine;
end

% if lw %| kill_neg

    fftMatrAt = (ifft2(fftshift((MatrA))));
    MS = (Exp.nPoints);
%     % ------------------ other way
%     MatrA = abs(fftshift(fft2(fftMatrAt(1:MS, 1:MS))));
%     fftGaga = Local_gaga2(1:MS, MS/2+0.5, 1:MS, MS/2+0.5, [1, 1]*lw/MS);
%         MatrA = conv2(MatrA, fftGaga);
%     
%     % ------------------ other way
...
%         fftGaga = abs(ifft2(fftshift(Local_gaga2(1:Exp.nPoints, Exp.nPoints/2+0.5, 1:Exp.nPoints, Exp.nPoints/2+0.5, [1, 1]*lw))));
        fftGaga = fftshift(Local_gaga2(1:Exp.nPoints*2, Exp.nPoints+0.5, 1:Exp.nPoints*2, Exp.nPoints+0.5, [1, 1]*Exp.nPoints*2/lw));
        fftMatrAt = fftMatrAt.*fftGaga/max(max(fftGaga));
    end
    % killing revers time domains
...
%--------------------------------------------------------
function [Amp, W] = Local_crosspeak(Str, tau, ii, kk, ll, nn, jj, mm, tDead)
E_a = Str.E_a;
E_b = Str.E_b;
% sn = size(E_a, 3);
I = Str.I;
...
for cc = 1:length(ii) % 1:nst^2*(cnt-1)^2 % 
    ...
    exppar = exp(-2i*pi*(wa(ii(cc), jj(cc)) + wb(ll(cc), mm(cc)))*tau) + exp(2i*pi*(wa(kk(cc), jj(cc)) + wb(nn(cc), mm(cc)))*tau);
    phasedr = exp(-2i*pi*(wa(ii(cc), kk(cc))+ wb(ll(cc), nn(cc)))*tDead ); %<<<<<<<<< alsi 19.09.2011 include deadtime
    Aikln =  (ampar.*exppar);
    Bikln =  (conj(ampar).*exppar);
     Amp(end+1, [1, 2]) = ([Aikln, Bikln])*phasedr; % changed alsi 15.09.2008  
