...
% Last Modified by GUIDE v2.5 29-Oct-2018 14:24:52
...
%         end
        ...
        hand.out{end}.y = -1*log10((ha.Ref.y)./(spl1.y(:, yones)));
        idx = find(hand.out{end}.y==Inf);
        hand.out{end}.y(idx) = 0;
...
if ha.showall
    for kk = 1:Num
        ...
        if safeget(trace, 'change2D', 0)
            ...
            hand.out{end}.y = real((trace.Data.y*zoom*trace.YSign - sh).*zm);
        else
            hand.out{end}.y = real(trace.Data.y*zoom*trace.YSign +trace.ShiftY); % <-alsi 04.10.2005
        end
...
if ~get(ha.fitBLasBL, 'Value')
    if length(ha.PTSidx) > 0
        ...
        makeitcomplex = 1; % for a convenience
        if makeitcomplex
            hand.out{end}.y = real(hand.out{end}.y) + i*real(hand.out{end}.y);
        end
...
[dataX, dataY] = Local_fftstuff(handles.Ref.ty,handles.Ref.ax.tx,  handles.Ref.dsc, Npoints);
...
function [x, y] = Local_fftstuff(iny, x, dsc1, Npoints)
...
if Npoints < 30 % it is not "NPoints", but a resolution in [cm-1]
    Npoints = 2*floor(HFlim/Npoints);
end

%%%%%%%%%%% figure out if backward scan exsist
tmpy1 = iny(1:(end/2), :);
...
if (PeakLoc-BPeakLoc)/numel(tmpy1)<0.1 %%% Backward and forward
    ...
    tx = [0:(Npoints*2)].';
    if AppodFunc == 1 % Blackman-Harris 3-term window
        Appod = 0.42323 - 0.49755*cos(2*pi*tx/(Npoints*2)) + 0.07922*cos(4*pi*tx/(Npoints*2));
    elseif AppodFunc == 2 % Norton-Beer, weak, medium, strong.
        % From https://doi.org/10.1364/AO.51.004622 
        %    Lu Lee, He Chen, Siying Chen, and Yinchao Zhang 
        %    Applied Optics Vol. 51, Issue 20, pp. 4622-4626 (2012)
        C(1, :) = [0.384093, -0.087577, 0.703484, 0, 0]; %weak (FWHM) = 1.2
        C(2, :) = [0.152442, -0.136176, 0.983734, 0, 0]; %medium (FWHM) = 1.4
        C(3, :) = [0.045335, 0, 0.554883, 0,  0.399782]; %strong (default in OMNIC) (FWHM) = 1.6
        Ch = 3;
        Appod = tx*0;
        for ii = 1:numel(C(Ch, :)), Appod = Appod+C(Ch, ii)*(1- ((tx-Npoints)/Npoints).^2).^(ii-1); end
    end
    
    data1 = [Zersss; Appod(:, ones(size(tmpy1, 2), 1)).*tmpy1([-Npoints:Npoints]+PeakLoc+1, :); Zersss];
    
    Mertz = 1;
    if Mertz % Mertz phase correction:
       ty = fft(data1, [], 1);
       The = atan2(imag(ty), real(ty));
       y = real(cos(The).*real(ty)+sin(The).*imag(ty));
    else
        y = abs(fft(data1, [], 1))*2;
    end
...
% --- Executes on button press in fitBLasBL.
function fitBLasBL_Callback(hObject, eventdata, handles)
