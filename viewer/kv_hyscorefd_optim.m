...
[phi, theta, turns, weigths] = Local_orisel(Sys, Exp, Opt);
...
for ii  =1:sum(nNucs)
    ...
    sIx{mult} = sop([Sys.I(ii)], 'x');
    sIy{mult} = sop([Sys.I(ii)], 'y');
    sIz{mult} = sop([Sys.I(ii)], 'z');
    ...
    %%%%%%%%%%%%%%% avoid wik =0 and wln=0
%     ixix = find(tii-tkk);
%     iyiy = find(tll(ixix)-tnn(ixix));
%     idx = ixix(iyiy);
% 
%     cii{ii} = tii(idx);
%     ckk{ii} = tkk(idx);
...
%             aFreqs = [aFreqs; frcs{ci}];
    if (ang>1)
        napr = 2;
        if (turns(ang-1)==turns(ang))
            for ci = Opt.Nuclei
                tamp = ampl{ci};
                tfrc = frcs{ci};
                ltamp = last_ampl{ci};
                ltfrc = last_frcs{ci};                
               for jj = 1:length(tamp) 
%                   damp = (tamp(jj, :) - ltamp(jj, :))/napr;
%                   dfreq = (tfrc(jj, :) - ltfrc(jj, :))/napr;
                  interamps(:, 1) = linspace(ltamp(jj, 1), tamp(jj, 1), napr)';
                  interfrqs(:, 1) = linspace(ltfrc(jj, 1), tfrc(jj, 1), napr)';
                  interamps(:, 2) = linspace(ltamp(jj, 2), tamp(jj, 2), napr)';
                  interfrqs(:, 2) = linspace(ltfrc(jj, 2), tfrc(jj, 2), napr)';                       
                  aAmps = [aAmps; interamps];
                  aFreqs = [aFreqs; interfrqs];
               end
            end
        else
            for ci = Opt.Nuclei
...
%     Amps = [Amps; aAmps]; %<<< memory consuming procedure alsi 08.06.06
%     Freqs = [Freqs; aFreqs];   
    if isempty(aFreqs)
        disp('ssss');
    end
    MatrA = MatrA + make_bin(aFreqs, aAmps, Exp); %alsi 08.06.06
%    outsize = size(Amps, 1);
end    
% disp(cputime - tstart);
...
%----------------------------------------------------
function MatrA = make_bin(Freqs, Amps, Exp)%, kill_neg, lw)
% tMatrA, tMatrB  = [Exp.Dim, Exp.Dim, sim_size] (see main function)
% kill_neg = 1 or 0. If 1, negative part of the timedomain "spectra" will
%   be == 0
% make_prod amplitude of the combination freq. peaks. if 0, product rull
...
%%%%%%%%%%%% check %%%%%%%%%%%%%%%%%%%%%%%
ind1 = [ind1, Exp.nPoints - ind1];
ind2 = [ind2, Exp.nPoints - ind2];
Amps = [Amps; Amps];

rMatrA = zeros(Exp.nPoints); iMatrA = rMatrA;

   rMatrA = bin2D(rMatrA, ind1(:), ind2(:), real(Amps(:, 1)+ Amps(:, 2)));
   rMatrA = bin2D(rMatrA, ind2(:), ind1(:), real(Amps(:, 1)+ Amps(:, 2)));
   
   iMatrA = bin2D(iMatrA, ind1(:), ind2(:), imag(Amps(:, 1)+ Amps(:, 2)));
   iMatrA = bin2D(iMatrA, ind2(:), ind1(:), imag(Amps(:, 1)+ Amps(:, 2)));
...
%   f_ind2 = [find1(find2), find1(find2)];
% 
%    rMatrA = bin2D(rMatrA, ind1(:), ind2(:), real(Amps(f_ind2, 1)+ Amps(f_ind2, 2)));
%    rMatrA = bin2D(rMatrA, ind2(:), ind1(:), real(Amps(f_ind2, 1)+ Amps(f_ind2, 2)));
%    
%    iMatrA = bin2D(iMatrA, ind1(:), ind2(:), imag(Amps(f_ind2, 1)+ Amps(f_ind2, 2)));
...
% 
% end
MatrA = rMatrA + i*iMatrA;
%----------------------------------------------------
function MatrA = make_fft(MatrA, Exp, kill_neg, lw)
% MatrA = abs(MatrA) + abs(MatrA.');
kill_zeros = 1;
if kill_zeros
        gauss = gaussian(1:(Exp.nPoints), Exp.nPoints/2, Exp.nPoints/1000);
...
Zeeman_A = -nu_n*(sintheta*cosphi*sIx{mult} + sintheta*sinphi*sIy{mult} + costheta*sIz{mult});
...
%%%%%% alsi 21.01.2008 %%%%%%%%%%%%%%%%%%%%%%%% VVVVVVVVVVVVVVVVVVV
...
%%%%%% alsi 21.01.2008 %%%%%%%%%%%%%%%%%%%%%%%% AAAAAAAAAAAAAAAAAAAAA
...
for cc = 1:length(ii) % 1:nst^2*(cnt-1)^2 % 
    % alsi 21.01.2008. moved to the optimisation box in main function
...
%     end
   % amppar = Mil*Mlj'*Mjn'*Mnk'*Mkm'*Mmi'
    ...
    ampar1 = cM(ii(cc), ll(cc)).*M(ll(cc), jj(cc)).*cM(jj(cc), nn(cc)).*M(nn(cc), kk(cc)).*cM(kk(cc), mm(cc)).*M(mm(cc), ii(cc));
%     CC = CC+1;
...
%      Amp(end+1, [1, 2]) = ([Aikln, Bikln]);
      Amp(end+1, [1, 2]) = ([real(Aikln)+imag(Aikln), real(Bikln)+imag(Bikln)]); % changed alsi 12.02.2008  
...
function [phi, theta, turns, weights] = Local_orisel(Sys, Exp, Opt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
turns = [];
